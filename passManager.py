...
if empty == True:
    MASTERPASS = input(Fore.LIGHTCYAN_EX + "Welcome. To use this program you must first set a master password for you account. This master password must contain at leat 1 lowercase, 1 uppercase, 1 number, and 1 special character. Your password must also be at least 8 characters long. Make sure this is something you can remember as you cannot change this later. Please enter your password here: " + Style.RESET_ALL)
    ...
    print(Fore.RED + Back.WHITE + "THIS IS VERY IMPORTANT! YOU MUST SAVE THIS KEY AS IT ALLOWS YOU TO USE THE MANAGER. SAVE THIS KEY SOMEWHERE: " + secret + Style.RESET_ALL)
    ...
    c.execute("INSERT INTO secrets (username, email, pass, secret) VALUES (%s, %s, %s, %s)", (user, email, USERPASS, secret))
    db.commit()
    ...
    if validate(MASTERPASS) == True: 
        ...
        while True:
            MASTERPASS = input(Fore.LIGHTCYAN_EX + "Welcome. To use this program you must first set a master password. This master password must contain at leat 1 lowercase, 1 uppercase, 1 number, and 1 special character. Your password must also be at least 8 characters long. Please enter your password here: " + Style.RESET_ALL)
...
while True:
    ...
    if int(what) == 1:
        ...
        c.execute("INSERT INTO passwords (site, username, password) VALUES (%s,%s,%s)", (site, user, passwd))
        db.commit()
        print(Fore.GREEN + "Successfully inserted data into table!" + Style.RESET_ALL)

        stop = input("Would you like to do more operations? (Y for yes | Any other key for no):")    
        if stop == "Y" or "Yes" or "y" or "yes": continue
        else: exit()
